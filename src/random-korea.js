...
module.exports = function(app) {
  app.get('/random/korea', async (req, res) => {
    try {
      ...
      // Ambil gambar dari API Korea
      const response = await fetch('https://api.agatz.xyz/api/korea');
      if (!response.ok) {
        return res.status(502).json({
          status: false,
          creator: "ubed bot",
          error: "Gagal mengambil gambar dari korea"
        });
      }
      ...
      if (!img.ok) {
        return res.status(502).json({
          status: false,
          creator: "ubed bot",
          error: "Gagal mengunduh gambar korea"
        });
      }
